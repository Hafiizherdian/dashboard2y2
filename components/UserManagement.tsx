...
function Spin({ sz=14 }: { sz?: number }) {
  return <svg style={{animation:'spin .65s linear infinite',width:sz,height:sz,flexShrink:0}} viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2.5" opacity=".2"/><path d="M4 12a8 8 0 018-8" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/></svg>;
}
...
function IBtn({ icon: Icon, color, bg, bd, onClick, title, disabled }: {
  ...
  return (
    <button onClick={onClick} title={title} disabled={disabled} className="um-btn" style={{width:28,height:28,borderRadius:5,background:bg,border:`1px solid ${bd}`,cursor:disabled?'not-allowed':'pointer',display:'flex',alignItems:'center',justifyContent:'center',opacity:disabled?.4:1,flexShrink:0}}>
...
// ─────────────────────────────────────────────────────────────────────────────────
// RESET PASSWORD MODAL — fitur baru
// Generates a temp password, shows it once, copies to clipboard
// Also has "send reset link" placeholder for email flow
// ─────────────────────────────────────────────────────────────────────────────────
function ResetPasswordModal({ user, theme, onClose, onReset }: {
  ...
  async function handleManual() {
    ...
    setSaving(true);
    try {
      await onReset(user.id, manualPw);
      setGenPw(manualPw);
      setStep('done');
    } catch { setError('Gagal mereset password.'); }
    finally { setSaving(false); }
  ...
  function copyToClipboard() {
    navigator.clipboard.writeText(genPw).then(() => {
      setCopied(true);
  ...
  const inputSt: React.CSSProperties = {
    ...
    background:t.input,border:`1px solid ${error?'#ef4444':t.inputBd}`,
        ...
        <div style={{padding:'18px'}}>
          ...
          {step === 'choose' && (
            <div style={{display:'flex',flexDirection:'column',gap:10}}>
              ...
              <button onClick={handleGenerate} disabled={saving} className="um-btn" style={{
                display:'flex',alignItems:'center',gap:12,padding:'12px 14px',borderRadius:8,
                border:`1px solid ${t.purpleBd}`,background:t.purpleBg,cursor:saving?'wait':'pointer',
              ...
              {/* Option 2: manual */}
              <button onClick={()=>setStep('manual')} className="um-btn" style={{
                display:'flex',alignItems:'center',gap:12,padding:'12px 14px',borderRadius:8,
                border:`1px solid ${t.blueBd}`,background:t.blueBg,cursor:'pointer',
                textAlign:'left',width:'100%',
              }}>
          ...
          {/* Step: done — show the generated/set password */}
          {step === 'done' && (
            <div style={{display:'flex',flexDirection:'column',gap:14}}>
              ...
              <div>
                ...
                <div style={{display:'flex',alignItems:'center',gap:6}}>
                  <div style={{flex:1,padding:'9px 12px',borderRadius:6,background:t.cardAlt,border:`1px solid ${t.lineStrong}`,fontFamily:F_MONO,fontSize:14,fontWeight:700,letterSpacing:'0.05em',color:t.tx1,wordBreak:'break-all'}}>
                    {genPw}
                  </div>
      ...
      <div style={{background:t.modal,border:`1px solid ${t.lineStrong}`,borderRadius:12,width:'100%',maxWidth:480,maxHeight:'90vh',display:'flex',flexDirection:'column',boxShadow:'0 20px 60px rgba(0,0,0,0.35)',overflow:'hidden',animation:'slideUp 0.18s ease'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 18px 14px',borderBottom:`1px solid ${t.line}`,flexShrink:0}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            ...
            <div>
              ...
              <div style={{fontSize:11,color:t.tx3,marginTop:1,fontFamily:F_MONO}}>
                {isEdit ? `@${editing!.username}` : 'Buat akun baru'}
              </div>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:14,padding:'18px',overflowY:'auto',color:t.tx3}}>
          ...
          <Field label="Role" hint={ROLE_CFG[form.role].desc}>
            <div style={{display:'flex',gap:6}}>
              {(['user','admin',...(me?.role==='root'?['root']:[])] as UserRole[]).map(r=>{
                ...
                return (
                  <button key={r} type="button" onClick={()=>setF('role',r)} style={{
                    flex:1,padding:'7px 6px',borderRadius:7,border:`2px solid ${sel?c.bd:t.line}`,
                    background:sel?c.bg:t.input,cursor:'pointer',
                    display:'flex',flexDirection:'column',alignItems:'center',gap:4,
                    transition:'all 0.12s',
                  }}>
                    <Icon size={13} color={sel?c.tx:t.tx3}/>
                    <span style={{fontSize:10,fontWeight:700,fontFamily:F_MONO,color:sel?c.tx:t.tx3,textTransform:'uppercase',letterSpacing:'0.05em'}}>
                      {ROLE_LABELS[r]}
                    </span>
                  </button>
                );
          ...
          <div style={{display:'flex',justifyContent:'flex-end',gap:7,marginTop:4}}>
            <button type="button" onClick={onCancel} className="um-btn" style={{padding:'8px 16px',borderRadius:7,fontSize:12,fontWeight:600,background:t.input,color:t.tx2,border:`1px solid ${t.inputBd}`,cursor:'pointer',fontFamily:F_SANS}}>
              Batal
            </button>
  ...
  return (
    <>
      ...
      <div style={{
        ...
        borderRadius:9,overflow:'hidden',boxShadow:t.shadow,
      }}>

        {/* Header */}
        <div style={{padding:'14px 18px',borderBottom:`1px solid ${t.line}`,background:t.cardAlt,display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            ...
            <div>
              <div style={{fontSize:14,fontWeight:800,color:t.tx1,fontFamily:F_SANS,letterSpacing:'-0.02em'}}>
                Manajemen User
              </div>
              <div style={{fontSize:10,color:t.tx3,fontFamily:F_MONO,marginTop:1}}>
        ...
        {/* Footer legend */}
        <div style={{padding:'11px 18px',borderTop:`1px solid ${t.line}`,background:t.cardAlt,display:'flex',gap:18,flexWrap:'wrap',alignItems:'center'}}>
