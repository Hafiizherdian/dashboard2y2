...
import {
  tk, Theme, FONT_MONO,
  iconBtn, Spinner, CardBox, FormGroup, ConfirmModal, StatCard,
} from './shared';
...
type SortKey = 'original_name' | 'created_at' | 'record_count' | 'area';
...
const PreviewPanel = React.memo(function PreviewPanel({
  ...
  useEffect(() => {
    ...
    setData([]);
    setAllCols([]);
    setActiveCols(new Set());

    fetch(`/api/distribution/${fileId}/preview`)
      .then(r => r.json())
      .then(r => {
  ...
  const stats = useMemo(() => {
    ...
    const planCol   = allCols.find(c => /^plan$/i.test(c));
    const aktualCol = allCols.find(c => /^aktual$/i.test(c));
    const kotaCol   = allCols.find(c => /^kota$/i.test(c));
    const produkCol = allCols.find(c => /^produk$/i.test(c));

    const totalPlan   = planCol   ? data.reduce((s, r) => s + Number(r[planCol]   ?? 0), 0) : null;
    const totalAktual = aktualCol ? data.reduce((s, r) => s + Number(r[aktualCol] ?? 0), 0) : null;
    const uniqueKota  = kotaCol   ? new Set(data.map(r => r[kotaCol])).size  : null;
    const uniqueProd  = produkCol ? new Set(data.map(r => r[produkCol])).size : null;

    return { totalPlan, totalAktual, uniqueKota, uniqueProd };
  }, [data, allCols]);
  ...
  const toggleCol = useCallback((col: string) => {
    setActiveCols(prev => {
      ...
      if (next.has(col)) {
        if (next.size > 2) next.delete(col);
      } else {
        next.add(col);
      }
      return next;
  ...
  const fmtCell = (col: string, val: unknown): string => {
    ...
    if (numericCols.includes(col) && !isNaN(Number(val))) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ minWidth: 0 }}>
            ...
            >
              {data.length} baris · {allCols.length} kolom
            </div>
          </div>
        </div>
        <span
      ...
      {stats &&
        (() => {
          const items = [
            ...
            ...(stats.totalPlan   !== null ? [{ label: 'Total Plan',   value: stats.totalPlan.toLocaleString('id-ID'),   sub: 'unit' }] : []),
            ...(stats.totalAktual !== null ? [{ label: 'Total Aktual', value: stats.totalAktual.toLocaleString('id-ID'), sub: 'unit' }] : []),
            ...(stats.uniqueKota  !== null ? [{ label: 'Kota / Area',  value: String(stats.uniqueKota),                  sub: 'wilayah' }] : []),
            ...(stats.uniqueProd  !== null ? [{ label: 'Produk Unik',  value: String(stats.uniqueProd),                  sub: 'SKU' }] : []),
          ...
          return (
            <button
              ...
              style={{
                ...
                background: on ? 'rgba(28,151,6,0.1)' : t.inputbg,
                color: on ? GREEN : t.textMuted,
                cursor: 'pointer',
                outline: 'none',
                whiteSpace: 'nowrap',
              }}
      ...
      <div style={{ overflowX: 'auto', maxHeight: 360, overflowY: 'auto' }}>
        <table
          style={{ minWidth: '100%', borderCollapse: 'collapse', fontSize: 12 }}
        >
          <thead>
            <tr>
              {visibleCols.map(col => (
                <th
                  ...
                  style={{
                    ...
                    zIndex: 2,
                    padding: '7px 12px',
                    textAlign: numericCols.includes(col) ? 'right' : 'left',
                    fontSize: 9,
                    fontWeight: 700,
                    fontFamily: FONT_MONO,
                    textTransform: 'uppercase',
          ...
          <tbody>
            {data.map((row, i) => (
              <tr
                key={i}
                style={{
                  background: i % 2 === 1 ? t.tableAlt : 'transparent',
                }}
              >
                {visibleCols.map(col => {
                  const numeric =
      ...
      >
        ...
        >
          {visibleCols.length} / {allCols.length} kolom
        </span>
        <span
          style={{
            fontSize: 10,
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <Eye size={15} color={t.green.text} />
            </div>
            <div style={{ minWidth: 0 }}>
              <div
                style={{ fontSize: 14, fontWeight: 700, color: t.text }}
              >
                Preview Data Distribusi
              </div>
              <div
                style={{
                  fontSize: 11,
        ...
        >
          <button
            ...
            style={{
              ...
              fontSize: 13,
              fontWeight: 600,
              background: t.gray.bg,
              color: t.gray.text,
              border: `1px solid ${t.gray.border}`,
              cursor: 'pointer',
            }}
          >
            Tutup
...
function FilesTable({
  ...
  const [previewFile, setPreviewFile] = useState<DistFile | null>(null);
  ...
  const thS = (key: SortKey): React.CSSProperties => ({
    ...
    color: key === sortKey ? GREEN : t.textMuted,
    borderBottom: `1px solid ${t.border}`,
    fontFamily: FONT_MONO,
    whiteSpace: 'nowrap',
    cursor: 'pointer',
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              style={{
                ...
                border: `1px solid ${search ? t.borderActive : t.borderInput}`,
                color: t.text,
                outline: 'none',
                width: isMobile ? 148 : 190,
                ...
                {TABLE_COLS.map(({ label, key }) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {key === sortKey ? (
                        sortDir === 'asc' ? (
                          <ChevronUp size={11} color={GREEN} />
                        ) : (
                          <ChevronDown size={11} color={GREEN} />
                        )
                      ) : (
              ...
              {paginated.length === 0 ? (
                ...
                paginated.map((file, idx) => (
                  <tr
                    ...
                    style={{
                      background:
                        idx % 2 === 1 ? t.tableAlt : 'transparent',
                    }}
                  >
                    {/* Nama File */}
                    <td
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...
                            textOverflow: 'ellipsis',
                            whiteSpace: 'nowrap',
                            maxWidth: isMobile ? 100 : 520, //lebar kolom file name
                          }}
                        >
                          {file.original_name}
                        </span>
                    ...
                    >
                      {file.record_count?.toLocaleString('id-ID')}
                    </td>

                    {/* Aksi */}
                    <td
                      style={{ padding: '11px 13px', textAlign: 'center' }}
                    >
                      <div
                        style={{
                          display: 'flex',
                      ...
                      >
                        <button
                          ...
                          style={iconBtn(t.green.bg, t.green.border, 28)}
                          title="Preview"
                        >
                          <Eye size={11} color={t.green.text} />
                        </button>
                        <button
                          onClick={() =>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
              ...
              <div style={{ display: 'flex', gap: 3 }}>
                <button
                  ...
                  disabled={safePage === 1}
                  style={{
                    ...iconBtn(t.gray.bg, t.gray.border, 26),
                        ...
                        style={{
                          ...
                          background:
                            safePage === item
                              ? 'rgba(28,151,6,0.12)'
                              : t.gray.bg,
                          color: safePage === item ? GREEN : t.textSub,
                          fontSize: 11,
                          fontFamily: FONT_MONO,
                          cursor: 'pointer',
...
export default function UploadDistribusiTab({
  theme,
  addToast,
  distFiles,
  onRefresh,
  ...
  const selectedAreaName =
    allAreas.find(a => a.id === selectedAreaId)?.name ?? selectedAreaId;
  ...
  const handleFileSelect = (file: File) => {
    ...
    if (/\.(xlsx|xls|csv)$/i.test(file.name)) {
      ...
      addToast('error', 'Format tidak didukung', 'Gunakan .xlsx, .xls, atau .csv');
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (res.ok && r.success) {
        ...
        setUploadMsg(msg);
        addToast('success', 'Upload distribusi berhasil!', msg);
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`/api/distribution?id=${deleteTarget.id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        ...
        addToast('error', 'Gagal menghapus', r.error);
      }
    } catch (err: any) {
      addToast('error', 'Gagal menghapus', err?.message);
    } finally {
      setIsDeleting(false);
  ...
  return (
    <>
      ...
      {isRoot && dbStats && (
        ...
        >
          <StatCard
            label="Total Records Distribusi"
          ...
          />
          <StatCard
            label="Total File Distribusi"
            value={(dbStats.distribution_files_count ?? 0).toLocaleString('id-ID')}
            cardKey="stat2"
            icon={FileSpreadsheet}
            sub={
              dbStats.distribution_latest_upload
                ? `Upload terakhir: ${new Date(dbStats.distribution_latest_upload).toLocaleDateString(
                    'id-ID',
                    { day: '2-digit', month: 'short', year: '2-digit' }
                  )}`
                : undefined
      ...
      {/* Upload card + required columns */}
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: isTablet ? '1fr' : '1fr 290px',
          gap: 16,
          marginBottom: 18,
          alignItems: 'start',
        }}
      >
        ...
        >
          ...
          {uploadStatus !== 'idle' && (
            <div
              style={{
                ...
                marginBottom: 12,
                background:
                  uploadStatus === 'success' ? t.green.bg : t.red.bg,
                border: `1px solid ${
                  uploadStatus === 'success' ? t.green.border : t.red.border
                }`,
            ...
            >
              ...
              <span
                style={{
                  fontSize: 12,
                  fontWeight: 600,
                  color: uploadStatus === 'success' ? t.green.text : t.red.text,
                  flex: 1,
                }}
              >
                {uploadMsg}
          ...
          <div
            ...
            style={{
              border: `2px dashed ${
                isDragging ? GREEN : selectedFile ? t.green.border : t.borderInput
              }`,
              borderRadius: 10,
              padding: selectedFile ? '14px' : '30px 14px',
              textAlign: 'center',
              ...
              <>
                <div
                  style={{
                    ...
                    height: 44,
                    borderRadius: 11,
                    background: isDragging ? 'rgba(28,151,6,0.12)' : t.inputbg,
                    border: `1.5px dashed ${isDragging ? GREEN : t.borderInput}`,
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    margin: '0 auto 10px',
          ...
          <FormGroup
            label="Area Tujuan"
            hint="Data distribusi akan dimasukkan ke area ini"
            theme={theme}
          >
            {allAreas.length === 0 ? (
              <div
              ...
              <div
                style={{
                  ...
                  border: `1px solid ${t.yellow.border}`,
                  borderRadius: 9,
                  fontFamily: FONT_MONO,
                }}
              >
              ...
              >
                <CheckCircle size={11} color={t.green.text} />
                {selectedAreaName}
              </div>
            ) : (
              <select
                ...
                style={{
                  fontSize: 12,
                  color: manualArea ? t.text : t.textMuted,
                  background: t.inputbg,
                  border: `1px solid ${manualArea ? GREEN_ACTIVE : t.borderInput}`,
                  borderRadius: 9,
                  fontFamily: FONT_MONO,
                  padding: '8px 12px',
                  width: '100%',
          ...
          {isUploading && (
            ...
            >
              ...
              <span
                style={{ fontSize: 12, color: t.green.text, fontFamily: FONT_MONO }}
              >
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                fontWeight: 700,
                border: 'none',
                background: canUpload ? GREEN : t.btnDisabled.bg,
                color: canUpload ? '#fff' : t.btnDisabled.text,
                cursor: canUpload ? 'pointer' : 'not-allowed',
                display: 'flex',
                alignItems: 'center',
                gap: 6,
                boxShadow: canUpload ? '0 2px 8px rgba(28,151,6,0.35)' : 'none',
            ...
            {REQUIRED_COLUMNS_DIST.map((col, i) => (
              <div
                ...
                style={{
                  ...
                  padding: '4px 2px',
                  borderBottom:
                    i < REQUIRED_COLUMNS_DIST.length - 1
                      ? `1px solid ${t.border}`
                      : 'none',
                }}
              >
                ...
                <span
                  style={{ fontSize: 11, fontFamily: FONT_MONO, color: t.textSub }}
                >
                  {col}
                </span>
              </div>
