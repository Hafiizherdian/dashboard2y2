...
const PreviewPanel = React.memo(function PreviewPanel({ fileId, fileName, fileStatus }: {
  fileId: string; fileName?: string; fileStatus?: UploadedFile['status'];
}) {
  const { t } = useTheme();
  ...
  const stats = useMemo(() => {
    ...
    const numericCols = allCols.filter(c => data.every(row => { const v = row[c]; return v !== '' && v !== null && v !== undefined && !isNaN(Number(v)); }));
  ...
  const statusColors: Record<UploadedFile['status'], { bg: string; text: string; border: string }> = {
    completed: t.green, processing: t.blue, error: t.red,
  };
  ...
  if (loading) return (
    <div style={{ padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 10, color: t.textMuted, fontSize: 12, fontFamily: FONT_MONO }}>
      <div style={{ display: 'flex', gap: 4 }}>
        {[0,1,2].map(i => <span key={i} style={{ width: 5, height: 5, borderRadius: '50%', background: t.textFaint, display: 'inline-block', animation: 'fadeIn 0.9s ease-in-out infinite', animationDelay: `${i*0.18}s` }}/>)}
      </div>
      Memuat preview…
    </div>
  );
  ...
  if (!data.length) return (
    <div style={{ padding: '28px', textAlign: 'center', color: t.textMuted, fontSize: 12, fontFamily: FONT_MONO }}>Tidak ada data untuk ditampilkan.</div>
  );
  ...
  return (
    <div style={{ borderRadius: 12, overflow: 'hidden', border: `1px solid ${t.border}`, background: t.cardbg }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: t.tableHead, borderBottom: `1px solid ${t.border}`, flexWrap: 'wrap', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0, flex: 1 }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 10, color: t.textMuted, fontFamily: FONT_MONO, marginTop: 1 }}>{data.length} baris &middot; {allCols.length} kolom</div>
          </div>
          {fileStatus && (
      ...
      {allCols.length > 0 && (
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', padding: '8px 12px', borderBottom: `1px solid ${t.border}`, background: t.tableHead }}>
          {allCols.map(col => {
            ...
            return (
              <button key={col} onClick={() => toggleCol(col)} title={on ? `Sembunyikan kolom ${col}` : `Tampilkan kolom ${col}`}
                style={{ fontSize: 10, fontFamily: FONT_MONO, padding: '2px 9px', borderRadius: 12, border: `1px solid ${on ? t.borderActive : t.border}`, background: on ? 'rgba(99,102,241,0.1)' : t.inputbg, color: on ? '#818cf8' : t.textMuted, cursor: 'pointer', transition: 'all 0.12s', outline: 'none', whiteSpace: 'nowrap' }}>
                {col}
              </button>
            );
          })}
      ...
      <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch', maxHeight: 360, overflowY: 'auto' }}>
        <table style={{ minWidth: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          <thead>
            <tr>
              {visibleCols.map(col => (
                <th key={col} style={{ position: 'sticky', top: 0, zIndex: 2, padding: '7px 12px', textAlign: 'left', fontSize: 9, fontWeight: 700, fontFamily: FONT_MONO, textTransform: 'uppercase', letterSpacing: '0.09em', color: t.textMuted, borderBottom: `1px solid ${t.border}`, background: t.tableHead, whiteSpace: 'nowrap', ...(stats?.numericCols.includes(col) ? { textAlign: 'right' } : {}) }}>
                  {col}
                </th>
              ))}
            </tr>
      ...
      <div style={{ padding: '7px 14px', borderTop: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: t.tableHead, flexWrap: 'wrap', gap: 6 }}>
        <span style={{ fontSize: 10, color: t.textMuted, fontFamily: FONT_MONO }}>{visibleCols.length} / {allCols.length} kolom ditampilkan</span>
        <span style={{ fontSize: 10, color: t.textFaint, fontFamily: FONT_MONO }}>klik label kolom di atas untuk toggle</span>
      </div>
    </div>
  );
});
...
function PreviewModal({ file, onClose }: { file: UploadedFile; onClose: () => void }) {
  ...
  return (
    <div onClick={e => { if (e.target === e.currentTarget) onClose(); }} style={{ position: 'fixed', inset: 0, zIndex: 1100, background: t.modalOverlay, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isMobile ? 10 : 24, animation: 'fadeIn 0.15s ease', backdropFilter: 'blur(4px)' }}>
      <div style={{ background: t.cardbg, border: `1px solid ${t.borderCard}`, borderRadius: 16, width: '100%', maxWidth: 1000, maxHeight: isMobile ? '92vh' : '85vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: t.shadowElevated, animation: 'slideUp 0.2s ease' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderBottom: `1px solid ${t.border}`, background: t.tableHead, flexShrink: 0, gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0, flex: 1 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: t.text, lineHeight: 1 }}>Preview Data</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            {!isMobile && (
              <>
                <span style={badge(t.gray.bg, t.gray.text, t.gray.border)}><Database size={9}/>{file.record_count.toLocaleString('id-ID')} records</span>
                <span style={badge(file.status==='completed'?t.green.bg:file.status==='error'?t.red.bg:t.blue.bg, file.status==='completed'?t.green.text:file.status==='error'?t.red.text:t.blue.text, file.status==='completed'?t.green.border:file.status==='error'?t.red.border:t.blue.border)}>
                  {file.status==='completed'&&<CheckCircle size={9}/>}{file.status==='error'&&<AlertCircle size={9}/>}{file.status}
                </span>
              </>
            )}
            <button onClick={onClose} title="Tutup (Esc)" style={{ width: 32, height: 32, borderRadius: 8, background: t.red.bg, border: `1px solid ${t.red.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0, transition: 'all 0.15s' }}>
              <X size={14} color={t.red.text}/>
            </button>
          </div>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '12px' : '16px 20px' }}>
          <PreviewPanel fileId={file.id} fileName={file.original_name} fileStatus={file.status}/>
        </div>
        <div style={{ padding: '10px 18px', borderTop: `1px solid ${t.border}`, background: t.tableHead, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0, flexWrap: 'wrap', gap: 8 }}>
          <span style={{ fontSize: 11, color: t.textFaint, fontFamily: FONT_MONO }}>Tekan <kbd style={{ padding: '1px 5px', borderRadius: 4, background: t.inputbg, border: `1px solid ${t.border}`, fontSize: 10, color: t.textMuted }}>Esc</kbd> atau klik di luar untuk menutup</span>
          <button onClick={onClose} style={{ padding: '7px 20px', borderRadius: 9, fontSize: 13, fontWeight: 600, background: t.gray.bg, color: t.gray.text, border: `1px solid ${t.gray.border}`, cursor: 'pointer', transition: 'all 0.15s' }}>Tutup</button>
        </div>
      </div>
    </div>
  );
...
function FilesTable({ files, onDelete, isRoot }: { files: UploadedFile[]; onDelete: (id: string, name: string) => void; isRoot: boolean }) {
  ...
  const pageNums = Array.from({length: totalPages}, (_,i)=>i+1)
    ...
    .reduce<(number|'ellipsis')[]>((acc,n,idx,arr)=>{ if(idx>0&&n-(arr[idx-1] as number)>1) acc.push('ellipsis'); acc.push(n); return acc; }, []);

  const visibleCols = isRoot ? TABLE_COLS : TABLE_COLS.filter(c=>c.key!=='record_count'&&c.key!=='total_omzet');
  const thS = (key: SortKey): React.CSSProperties => ({ padding: '10px 13px', textAlign: 'left', fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: key===sortKey?'#6366f1':t.textMuted, borderBottom: `1px solid ${t.border}`, fontFamily: FONT_MONO, whiteSpace: 'nowrap', cursor: 'pointer', userSelect: 'none', background: t.tableHead });
      ...
      <div style={card(t, { overflow: 'visible' })}>
        <div style={{ padding: '14px 16px', borderBottom: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: t.textMuted, marginTop: 2, fontFamily: FONT_MONO }}>{filtered.length!==files.length?`${filtered.length} / ${files.length}`:`${files.length} total`}</div>
          </div>
            ...
            <tbody>
              {paginated.length===0 ? (
                <tr><td colSpan={visibleCols.length+1} style={{padding:40,textAlign:'center',color:t.textMuted,fontSize:12,fontFamily:FONT_MONO}}>
                  <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:8}}><FileSpreadsheet size={26} color={t.textFaint}/>{search?`Tidak ada "${search}"` : 'Belum ada file'}</div>
                </td></tr>
                ...
                <tr key={file.id} style={{background:idx%2===1?t.tableAlt:'transparent'}}>
                  <td style={{padding:'11px 13px',color:t.text,fontWeight:500}}>
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      <div style={{width:26,height:26,borderRadius:6,background:t.green.bg,border:`1px solid ${t.green.border}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><FileSpreadsheet size={12} color={t.green.text}/></div>
                  ...
                  <td style={{padding:'11px 13px',textAlign:'center'}}>
                    <div style={{display:'flex',justifyContent:'center',gap:5}}>
                      <button onClick={()=>setPreviewFile(file)} style={iconBtn(t.blue.bg,t.blue.border,28)} title="Preview data"><Eye size={11} color={t.blue.text}/></button>
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (res.ok&&(r.success!==false)) {
        ...
        setSelectedFile(null); setManualArea(''); if(fileInputRef.current) fileInputRef.current.value='';
    ...
    } catch(err:any) { const msg=err?.message||'Koneksi gagal, coba kembali'; setUploadStatus('error'); setUploadMsg(msg); addToast('error','Upload gagal',msg); }
    ...
    <>
      ...
      <div style={{display:'grid',gridTemplateColumns:isTablet?'1fr':'1fr 290px',gap:16,marginBottom:18,alignItems:'start'}}>
        <CardBox title="Upload File Baru" icon={Upload} iconColor="#6366f1" accent="#6366f1">
          <StatusBar/>
          <div onDragOver={e=>{e.preventDefault();setIsDragging(true);}} onDragLeave={e=>{e.preventDefault();setIsDragging(false);}} onDrop={handleDrop} onClick={()=>!selectedFile&&fileInputRef.current?.click()}
            ...
            {!selectedFile?(
              ...
              <div style={{display:'flex',alignItems:'center',gap:12,textAlign:'left'}}>
                <div style={{width:38,height:38,borderRadius:9,background:t.green.bg,border:`1px solid ${t.green.border}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><FileSpreadsheet size={18} color={t.green.text}/></div>
                <div style={{flex:1,minWidth:0}}>
...
function ResetPasswordForm() {
  ...
  const strength = useMemo(() => {
    ...
    if (newPw.length>=8) s++; if (/[A-Z]/.test(newPw)) s++; if (/[0-9]/.test(newPw)) s++; if (/[^A-Za-z0-9]/.test(newPw)) s++;
    return s;
  }, [newPw]);
  ...
  const inputStyle = (): React.CSSProperties => ({ width:'100%',padding:'9px 38px 9px 12px',fontSize:13,borderRadius:9,background:t.inputbg,border:`1px solid ${t.borderInput}`,color:t.text,outline:'none',fontFamily:FONT_MONO,transition:'border-color 0.15s' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok&&r.success) { addToast('success','Password berhasil diubah','Silakan login kembali dengan password baru'); setCurrentPw('');setNewPw('');setConfirmPw(''); }
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      <FormGroup label="Password Saat Ini">
        <div style={{position:'relative'}}><input type={showCur?'text':'password'} value={currentPw} onChange={e=>setCurrentPw(e.target.value)} placeholder="Masukkan password saat ini" style={inputStyle()}/><EyeToggle show={showCur} onToggle={()=>setShowCur(p=>!p)}/></div>
      </FormGroup>
      <FormGroup label="Password Baru" hint="Minimal 6 karakter, kombinasikan huruf dan angka">
        <div style={{position:'relative'}}><input type={showNew?'text':'password'} value={newPw} onChange={e=>setNewPw(e.target.value)} placeholder="Masukkan password baru" style={inputStyle()}/><EyeToggle show={showNew} onToggle={()=>setShowNew(p=>!p)}/></div>
        {newPw.length>0&&(
          <div style={{marginTop:7}}>
            <div style={{display:'flex',gap:4,marginBottom:4}}>{[1,2,3,4].map(lvl=><div key={lvl} style={{flex:1,height:3,borderRadius:3,background:lvl<=strength?strengthColor:t.border,transition:'background 0.2s'}}/>)}</div>
      ...
      <FormGroup label="Konfirmasi Password Baru">
        <div style={{position:'relative'}}><input type={showCon?'text':'password'} value={confirmPw} onChange={e=>setConfirmPw(e.target.value)} placeholder="Ulangi password baru" style={inputStyle()}/><EyeToggle show={showCon} onToggle={()=>setShowCon(p=>!p)}/></div>
        {confirmPw&&newPw&&(
          <div style={{marginTop:5,fontSize:11,fontFamily:FONT_MONO,display:'flex',alignItems:'center',gap:4,color:confirmPw===newPw?t.green.text:t.red.text}}>
            {confirmPw===newPw?<CheckCircle size={10}/>:<X size={10}/>}{confirmPw===newPw?'Password cocok':'Password tidak cocok'}
          </div>
        )}
      </FormGroup>
      <button onClick={handleSubmit} disabled={loading||!currentPw||!newPw||!confirmPw||newPw!==confirmPw}
        style={{padding:'10px 20px',borderRadius:9,fontSize:13,fontWeight:700,border:'none',background:(loading||!currentPw||!newPw||!confirmPw||newPw!==confirmPw)?t.btnDisabled.bg:'#6366f1',color:(loading||!currentPw||!newPw||!confirmPw||newPw!==confirmPw)?t.btnDisabled.text:'#fff',cursor:(loading||!currentPw||!newPw||!confirmPw||newPw!==confirmPw)?'not-allowed':'pointer',display:'flex',alignItems:'center',gap:8,boxShadow:(loading||!currentPw||!newPw||!confirmPw)?'none':'0 2px 10px rgba(99,102,241,0.3)',transition:'all 0.15s',alignSelf:'flex-start'}}>
...
function SettingsTab() {
  ...
  const handleMigrate = async () => {
    try {
      const res = await fetch('/api/migrate',{method:'POST'}); const r=await res.json();
      if(r.success) addToast('success','Migration selesai!'); else addToast('error','Migration gagal',r.error);
    } catch { addToast('error','Gagal','Coba kembali.'); }
  };

  return (
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(320px, 1fr))',gap:16,alignItems:'start'}}>
      <CardBox title="Ubah Password" icon={Lock} iconColor="#6366f1" accent="#6366f1"><ResetPasswordForm/></CardBox>
      {isRoot&&(
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          <CardBox title="Database Migration" icon={Database} iconColor="#10b981" accent="#10b981">
            ...
            <button onClick={handleMigrate} style={{padding:'9px 20px',borderRadius:9,fontSize:13,fontWeight:600,background:'#10b981',color:'#fff',border:'none',cursor:'pointer',boxShadow:'0 2px 8px rgba(16,185,129,0.3)',display:'flex',alignItems:'center',gap:8}}><Database size={14}/> Run Migration</button>
          </CardBox>
    ...
    <button onClick={onClick} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)} title={collapsed?label:undefined}
      ...
      {!collapsed&&(
        <>
          ...
          {bdg!==undefined&&bdg>0&&<span style={{fontSize:10,fontWeight:700,fontFamily:FONT_MONO,background:accent,color:'#fff',padding:'1px 7px',borderRadius:12,boxShadow:`0 2px 6px ${accent}50`}}>{bdg}</span>}
        </>
      )}
      {collapsed&&hovered&&(
      ...
      <div style={{borderTop:`1px solid ${t.sidebarBorder}`,padding:collapsed?'10px 6px':'10px 10px',flexShrink:0,display:'flex',flexDirection:'column',gap:6}}>
        ...
        {collapsed ? (
          <button onClick={()=>setTheme(theme==='dark'?'light':'dark')} title={theme==='dark'?'Mode Terang':'Mode Gelap'}
            style={{width:'100%',height:34,borderRadius:8,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.08)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'rgba(255,255,255,0.5)',transition:'all 0.15s'}}>
            {theme==='dark'?<Sun size={14}/>:<Moon size={14}/>}
          </button>
        ) : (
          <button onClick={()=>setTheme(theme==='dark'?'light':'dark')}
            style={{width:'100%',display:'flex',alignItems:'center',gap:8,padding:'8px 10px',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:8,cursor:'pointer',transition:'all 0.15s',fontSize:12,fontWeight:500,fontFamily:FONT_SANS}}
        ...
        {collapsed ? (
          <button onClick={logout} title="Logout"
            style={{width:'100%',height:34,borderRadius:8,background:'rgba(239,68,68,0.1)',border:'1px solid rgba(239,68,68,0.2)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',transition:'all 0.15s'}}>
            <LogOut size={13} color="#f87171"/>
          </button>
        ) : (
          <button onClick={logout}
            style={{width:'100%',display:'flex',alignItems:'center',gap:8,padding:'8px 10px',background:'rgba(239,68,68,0.08)',border:'1px solid rgba(239,68,68,0.18)',borderRadius:8,cursor:'pointer',transition:'all 0.15s',fontSize:12,fontWeight:600,fontFamily:FONT_SANS}}
  ...
  if (isLoading) return (
    <div style={{minHeight:'100vh',background:'#07090e',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',fontFamily:'IBM Plex Mono, monospace'}}>
      <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:20}}>
        ...
        <div style={{fontSize:10,color:'rgba(255,255,255,0.22)',letterSpacing:'0.06em'}}>Memuat data…</div>
      </div>
    </div>
  );

  return (
    <div style={{height:'100vh',background:t.pagebg,fontFamily:FONT_SANS,display:'flex',overflow:'hidden',transition:'background 0.3s'}}>
      <Sidebar
